...
correct.ratio <- array(0,c(6,Rep,2));
se.record <- array(0,c(6,Rep,2));
time.record <- matrix(0, Rep, 2);
...
for( N in c(100,200) ){
    ...
    for( TT in c(10, 20, 40) ){
        ...
        for( r in 1:Rep ){
            ...
            y <- d$y; X <- d$X; lambda <- as.numeric( 0.5 * var(y) / (TT^(1/3)) );
            
            t.begin <- Sys.time();
            pls <- PLS.mosek(N, TT, y, X, K, lambda, MaxIter);
            t.end <- Sys.time(); time.record[r,1] <- t.end - t.begin; time.mosek <- time.mosek + ( t.end - t.begin );
            print(time.mosek)
            ...
            # t.end <- Sys.time(); time.record[r,2] <- t.end - t.begin; time.nlopt <- time.nlopt + ( t.end - t.begin );
            # print(time.nlopt)
            # result.temp <- group.coerce(pls$group.est, pls$a.out, group0, t(a0), N, N.frac, K, p );
            ...
            t.end <- Sys.time(); time.record[r,2] <- t.end - t.begin; time.cvxr <- time.cvxr + ( t.end - t.begin );
            print(time.cvxr)
            result.temp <- group.coerce(pls$group.est, pls$a.out, group0, t(a0), N, N.frac, K, p );
            correct.ratio[case, r, 2] <- result.temp$ratio;
            se.record[case, r, 2] <- result.temp$se;
...
print("mosek result:")
print( report(correct.ratio[ , , 1], se.record[ , , 1], Rep) )
print( time.mosek )
print("cvxr result:")
print( report(correct.ratio[ , , 2], se.record[ , , 2], Rep) )
print( time.nlopt )
