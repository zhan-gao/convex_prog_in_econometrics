## ECOS solver (hooked with CVXR) does not have stable performance It
## sometimes sensitive to data input.  For comparison, we ignore cases in
## which ECOS breaks down when generate data.
    ...
    for (TT in c(15, 25, 50)) {
        ...
        while (r <= Rep) {
            ...
            tryCatch({
                ...
                # filename = paste0('./CLasso/simu_data/pls_data_N_', N, '_T_', TT, '_r_', r,
                # '.csv') D = read.csv(filename, header = TRUE)
                ...
                file.name = paste0("./CLasso/simu_data_full/pls_data_N_", N, 
                  "_T_", TT, "_r_", r, ".csv")
...
write.csv(rbind(rmse, cr, time.sum), "PLS_Result.csv")
save.image(file = "PLS_Result.RData")
